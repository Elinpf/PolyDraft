...
| 变量 | 来源 | 示例值 |
|------|------|--------|
| `{品牌}` | 生成页"补充输入" | 用户自定义品牌名 |
| `{产品系列}` | 选项维度"产品系列"（注入 label） | 该系列 label |
| `{产品段位}` | 选项维度"产品段位" | 1 |
| `{文案类型}` | 选项维度"文案类型" | 该类型 label |
| `{文案类型提示词}` | "文案类型"选项的 prompt 片段 | 该选项配置的提示词片段 |
| `{喂养场景}` | 选项维度"喂养场景"（注入 label） | 该场景 label |
| `{喂养场景提示词}` | "喂养场景"选项的 prompt 片段 | 该选项配置的提示词片段 |
